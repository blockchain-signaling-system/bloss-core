...
class AttackReporting:
    ...
    def parse_attack_report_from_node(self, message):
        target = message['target']
        action = message['action']
        timestamp = message['timestamp']
        subnetwork = str(message['subnetwork'])
        addresses = set(message['addresses'])
        hash = message['hash']
    ...
    def parse_attack_report_message(self, message):
        ...
        for key, value in message.iteritems():
            if key == "target":
                ...
                current_timestamp = datetime.now()
                delta_timestamp_seconds = (current_timestamp - timestamp).total_seconds()
                if (delta_timestamp_seconds >= self._config['INTERVAL']['MESSAGE_LIFETIME_SECONDS']):
                    return None
            elif key == "subnetwork":
